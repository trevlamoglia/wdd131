...
function search() {
  let recipeQuery = input.value.trim().toLowerCase();
  ...
  if (filterRecipes.length === 0) {
    recipeContainer.innerHTML = '<p class="no-results">No recipes matched your search.</p>';
    return;
  }

  filterRecipes.forEach(function(recipe) {
    renderRecipe(recipe);
  });
}
  ...
  return `
    <article class="recipe-card">
      ...
      <div class="recipe-content">
        <div class="recipe-category">${tagTemplate(recipe.tags)}</div>
        <h2>${recipe.name}</h2>
        <p class="recipe-rating">${ratingStars}</p>
